...
function generateListPage(messages) {
  const rows = messages.map(msg => `
    <tr><td>${escapeHtml(msg.mailbox_email)}</td><td>${escapeHtml(msg.from_address)}</td>
    <td><a href="/view/${msg.id}">${escapeHtml(msg.subject)}</a></td>
    <td>${new Date(msg.received_at).toLocaleString()}</td><td>${msg.is_read ? '已读' : '未读'}</td></tr>
  `).join('');
  ...
  .logout-btn:hover{background:#c82333;}</style></head>
...
function generateDetailPage(message) {
  ...
  .email-content{white-space:pre-wrap;word-wrap:break-word;}.email-content img{max-width:100%;}</style></head>
